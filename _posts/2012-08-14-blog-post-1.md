...
We'll first start with value iteration, as I believe it is the easier to understand conceptually, at least initially. I'll show the algorithm, then talk about each of the steps:

1. Start with model $p(s',\|s,a)$
2. Initialize value function $v(s)=0$ for all states $s$
3. do:
4. &emsp; for $s\in S$:
5. &emsp; &emsp; $v'(s) = \text{max}_a \sum _{s'\in S} P(s',r\|s,a)\left( r + \gamma v(s') \right)$
6. while $\|v'(s) - v(s)\|_1 < \text{small number}$
7. return v(s)

In order to show what this algorithm does, lets look at a simple example. Below is a 1D world comprised of seven squares.  Our goal is to reach the rightmost square, and avoid the leftmost square. In accordance with this goal, we set the following reward of +10 if we reach the right sqaure, and a reward of -1 if we reach the left square. All other state-action pairs result in a reward of zero. Inside each square is the value for $v(s)$, which we initialize to zero everywhere:

![Initial Value Function](https://jacobhiggins.github.io/_posts/post_pics/bp1/fig1.png)
